...
from News import get_news, process_news_selection # Import the new function
...
def main():
    ...
    # State variable to check if we are waiting for a news number
    ...
    while True:
        ...
        # --- New Logic: Check if we are waiting for a news number ---
        if is_waiting_for_news_selection:
            if process_news_selection(command, bolo):
                is_waiting_for_news_selection = False # Exit selection mode
            else:
        ...
        if any(phrase in command for phrase in Config.goodbye_triggers):
            ...
            # get_news now returns True if it's waiting for a number
